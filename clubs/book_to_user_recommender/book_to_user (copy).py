...
class BookToUserRecommender:
    ...
    def get_top_books(self):
        ...
        top_20_books = self.df_ratings.groupby('id').agg(['mean', 'count'])['rating'].reset_index()
        ...
        # get title of books
        top_20_books = pd.merge(top_20_books, self.df_books, on='id')[['title', 'author', 'mean', 'count', 'weighted rating',
    ...
    def get_top_authors(self):
        ...
        # highest_rated_author = pd.merge(self.df_books, self.df_ratings, on='ISBN')[['author', 'Book-Rating', 'title', 'ISBN']]
        ...
        highest_rated_author['weighted rating'] = (vote_count_author / (vote_count_author + min_votes_author)) * rating_mean_author + (min_votes_author / (vote_count_author + min_votes_author)) * C
